...
<div class="h-popup" v-if="isShow">
    <popup @closePopup="closePopup" :width="600" :height="500">
        ...
        <template slot="content">
            <div>
                <div class="h-row h-display-flex h-mg-t10">
                    <div class="h-col h-flex">
                        <Input :title="'Tiêu đề tin tức'" placeholder="Tiêu đề tin tức" v-model="news.newsTitle" />
                    </div>
                </div>
                <div class="h-row h-display-flex">
                    <div class="h-col h-flex">
                        <Textarea :title="'Nội dung tin tức'" placeholder="Nội dung tin tức" v-model="news.newsContent" />
                        </div>
          </div>
          <div class="h-row h-display-flex">
            <div class="h-col h-flex">
              <InputFile :title="'Ảnh tin tức'" ref="inputFile" :name="news.linkImage"/>
            </div>
          </div>
          <div class="h-row h-display-flex">
            <div class="h-col h-flex">
          ...
          <div class="h-row h-display-flex">
            <div class="h-col h-flex">
              <Input :title="'Tin khuyến mại'"
               placeholder="Tin khuyến mại"
                v-model="news.newsSale"
                />
            </div>
            <div class="h-col h-flex ml-10">
              <Input :title="'Tin sự kiện'"
...
export default {
    ...
    methods: {
        ...
        async saveNews() {
            ...
            me.news.linkImage = commonFunc.convertImgSaveDB(me.$refs.inputFile.fileName);
            // Thêm
            if (me.mode == "add") {
